 ...
 */
        ...
        *:after {
        ...
        .tblRpt.tblRpt-bordered>thead>tr>th,
        .tblRpt.tblRpt-bordered>tbody>tr>td,
        .tblRpt.tblRpt-bordered tfoot>tr>td {
            border: 1px solid #d1d1d1
        }

        .tblRpt.tblRpt-striped tbody>tr:nth-of-type(2n) {
            background-color: #eae9ee
        }

        .tblRpt.alternative-row-table tbody .even-row {
            background-color: #eae9ee
        }

        .tblRpt.tblRpt-hover>tbody>tr:hover>td {
        ...
        .tblRpt thead tr.thead-row2-pt th {
            background-color: #f0f1c7;
            color: #333
        }

        .tblRpt thead tr.thead-row2-pt th.thead-row {
            background-color: #666;
            color: #fff
        }

        .tblRpt .thead-row2 th {
            background-color: #3f3f3f;
            color: #fff
        }

        .tblRpt tbody td.text-center {
            text-align: center
        }

        .tblRpt tbody td.group-league {
            background: #f4f5e0;
            text-transform: uppercase;
            color: #333;
            font-weight: bold;
        ...
        .tblRpt tbody .col-first {
            background-color: #ffefde
        }

        .tblRpt tbody .altercol {
            background-color: #f6f2d7;
            text-align: right
        }

        .tblRpt tbody .col-sma {
        ...
        .tblRpt .product-row .altercol,
        .tblRpt .pt-row .altercol {
            background-color: #f3f5e0
        }

        .tblRpt .product-row:hover,
        .tblRpt .pt-row:hover {
            cursor: pointer
        }

        .tblRpt .product-row td:first-child,
        .tblRpt .pt-row td:first-child {
            width: 20%
        }

        .tblRpt .product-row>td .icon-arrow-right,
        .tblRpt .pt-row>td .icon-arrow-right {
        ...
        .tblRpt .product-row>td.active .icon-arrow-right,
        .tblRpt .pt-row>td.active .icon-arrow-right {
        ...
        .combined-fixed-pagination-bottom {
            margin-bottom: 48px
        }
...
<body>
    ...
    <div class="bl_title">
        Danh sách cược thành viên {{ $req->start_date }} --&gt; {{ $req->end_date }} - {{ $user->username }}
    </div>
    <table class="tblRpt tblRpt-bordered tblRpt-striped width-100per tablesorter tblRpt-hover tbl-betlist">
        <thead>
            <tr>
                ...
                <th class="col-winloss">Thắng thua</th>
        ...
        <tbody>
            @foreach ($bets as $key => $value)
                @if (!$value->is_group)
                    <tr id="{{ $value->id }}" class="{{ $value->bet_status_name == 'Cược bất thường' ? 'background-cbt' : '' }}">
                        ...
                        <td class="text-right sport-1 {{ $value->bet_status_name == 'Cược bất thường' ? 'text-decoration' : '' }}">
                            <div class="">
                                ...
                                @else
                                <span class="underdog">{{ $value->bet_name }}<span
                                        class="handicap">
                        ...
                        <td class="win-loss-block text-right nonbreak">
                            <div>
                                <span class="bl_underdog">
                                    <font color="{{ $value->bet_profit < 0 ? '#B50000' : '' }}">
                                        {{ $value->bet_profit }}</font>
                                </span>
                            </div>
                            <div>
                                <span class="bl_underdog">{{ number_format($value->bet_commission, 2) }}</span>
                            </div>
                        </td>
                        <td class="status-block text-center sport-1">
                            <div class="status"> {{ $value->bet_status_name }}</div>
                            <div class="result"
                        ...
                        <td class="position-taking-block text-right">
                            <div class="position-taking bl_underdog">
                                <font class="pt-value">0%</font><br><span class="pt-winloss">0.00</span><br>
                            </div>0.25%<br>0.00<br>
                        </td>
                ...
                @else
                    <tr id="{{ $value->id }}">
                        ...
                        <td class="r bl_evt">
                            <div>
                                <a href="javascript:showP('{{ $value->id }}')" onclick="" id="hidden0">Cược tổng
                                    hợp</a><br>
                                <br>
                                    ...
                                    @foreach ($value->bets as $index => $bet)
                                        <div class="ticketList">
                                            <div class="">
                                                ...
                                                <div class="league">
                                                    <span class="sport">Bóng đá</span><span
            ...
            <tr class="bl_footertotal">
                ...
                <td class="text-right">
                    <div>
                        <font color="{{ $sum['profit'] < 0 ? '#B50000' : '' }}">{{ $sum['profit'] }}</font>
                    </div>
                    <div>{{ $sum['commission'] }}</div>
                    <div>
                        <font color="{{ $sum['total'] < 0 ? '#B50000' : '' }}">{{ $sum['total'] }}</font>
                    </div>
                </td>
                <td colspan="2"></td>
            </tr>
    ...
    <div class="comm_note">
        ...
        <div class="content">
            Để tiện cho việc tham khảo, giá trị Hoa Hồng sẽ được làm tròn. Do đó, giá trị Tổng phụ (hoa hồng) có thể sẽ
            khác với tổng của các giá trị Hoa Hồng.
            Tiền tích lũy sẽ được làm tròn đến 4 số lẻ.
        </div>
    </div>
</body>
